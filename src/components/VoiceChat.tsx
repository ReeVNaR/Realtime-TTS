...
export default function VoiceChat() {
            ...
            if (SpeechRecognition) {
                ...
                recognition.onerror = (event: any) => {
                    ...
                    if (event.error === 'not-allowed') {
                        setBotReply("Error: Microphone access denied. Please allow permissions.");
                ...
                setBotReply("Error: Speech Recognition not supported in this browser.");
            }
        }
    ...
    const toggleListening = () => {
        // Unlock audio context and element for mobile
        resumeAudioContext();
        if (audioRef.current) {
        ...
        if (isSessionActive) {
            ...
            try {
                ...
                // If already started, ignore. If other error, maybe alert user.
                if (e.message !== "Failed to execute 'start' on 'SpeechRecognition': recognition has already started.") {
                    setBotReply(`Error: ${e.message || 'Microphone access failed'}`);
                }
            }
        }
    };
    ...
    return (
        <div className="min-h-screen bg-[#020205] text-white overflow-hidden relative font-sans flex flex-col">

            {/* Tailwind Background Effects */}
            <div className="absolute top-0 left-0 w-64 h-64 md:w-96 md:h-96 bg-purple-600/20 rounded-full blur-3xl -translate-x-1/2 -translate-y-1/2 animate-pulse" />
            <div className="absolute bottom-0 right-0 w-64 h-64 md:w-96 md:h-96 bg-blue-600/20 rounded-full blur-3xl translate-x-1/2 translate-y-1/2 animate-pulse delay-1000" />
            <div className="absolute top-1/2 left-1/2 w-[500px] h-[500px] md:w-[800px] md:h-[800px] bg-indigo-900/10 rounded-full blur-3xl -translate-x-1/2 -translate-y-1/2 pointer-events-none" />

            {/* Header */}
            <header className="relative z-10 p-4 md:p-6 flex justify-between items-center backdrop-blur-sm border-b border-white/5">
                <div className="flex items-center space-x-2">
                    <Sparkles className="w-4 h-4 md:w-5 md:h-5 text-purple-400" />
                    <h1 className="text-lg md:text-xl font-bold tracking-tighter bg-clip-text text-transparent bg-gradient-to-r from-purple-400 to-blue-500">
                        AETHER
                    </h1>
                </div>
                <div className="flex items-center space-x-2 bg-white/5 rounded-full px-2 py-1 md:px-3 md:py-1.5 border border-white/10">
                    <Radio className="w-3 h-3 text-purple-400" />
                    <select value={voice} onChange={(e) => setVoice(e.target.value)} className="bg-transparent text-[10px] md:text-xs text-gray-300 focus:outline-none uppercase max-w-[80px] md:max-w-none">
            ...
            {/* Main Content */}
            <div className="flex-1 relative z-10 flex flex-col items-center justify-center p-4 w-full max-w-full">

                {/* Central Reactive Core (SVG) */}
                <div className="relative mb-8 md:mb-16 cursor-pointer group" onClick={toggleListening}>
                    <div className="relative w-[160px] h-[160px] md:w-[200px] md:h-[200px] flex items-center justify-center">
                        {/* Outer Rings */}
                        <div className={clsx(
                        ...
                        {/* Inner Core */}
                        <div className={clsx(
                            "relative w-16 h-16 md:w-20 md:h-20 rounded-full flex items-center justify-center transition-all duration-300 shadow-[0_0_30px_rgba(139,92,246,0.3)]",
                            ...
                            {status === 'listening' ? <Mic className="w-6 h-6 md:w-8 md:h-8 text-white" /> :
                                status === 'thinking' ? <Loader2 className="w-6 h-6 md:w-8 md:h-8 text-white animate-spin" /> :
                                    status === 'speaking' ? <Volume2 className="w-6 h-6 md:w-8 md:h-8 text-white" /> :
                                        <Mic className="w-6 h-6 md:w-8 md:h-8 text-white/50" />}
                        </div>
                    </div>

                    <div className="absolute -bottom-8 left-1/2 -translate-x-1/2 whitespace-nowrap">
                        <span className="text-[10px] md:text-xs font-mono text-purple-300/70 tracking-[0.3em] uppercase">
                ...
                {/* Text Display */}
                <div className="w-full max-w-2xl text-center min-h-[100px] md:min-h-[120px] flex flex-col justify-center space-y-2 md:space-y-4 px-2">
                    {transcript && (
                        <p className="text-lg md:text-3xl font-light text-white/80 leading-tight tracking-tight break-words">
                            "{transcript}"
                        </p>
                    )}
                    {botReply && (
                        <div key={botReply} className="text-base md:text-2xl font-medium text-transparent bg-clip-text bg-gradient-to-r from-purple-200 to-blue-200 leading-relaxed animate-fade-in-up break-words">
                            {botReply}
                        </div>
                    )}
                </div>
            ...
            {/* Footer Input */}
            <div className="relative z-10 p-4 md:p-6 w-full max-w-xl mx-auto">
                <form onSubmit={handleTextSubmit} className="relative">
                    <div className="absolute inset-0 bg-gradient-to-r from-purple-600 to-blue-600 rounded-full opacity-20 blur-md"></div>
                    <div className="relative flex items-center bg-black/60 backdrop-blur-xl border border-white/10 rounded-full px-4 py-2 md:px-6 md:py-3 shadow-2xl">
                        <MessageSquare className="w-3 h-3 md:w-4 md:h-4 text-gray-500 mr-2 md:mr-4" />
                        <input
                            type="text"
                            value={textInput}
                            ...
                            className="flex-1 bg-transparent text-white placeholder-gray-600 focus:outline-none text-xs md:text-sm font-mono w-full"
                        />
                        <button
                            type="submit"
                            disabled={!textInput.trim()}
                            className="ml-2 p-1.5 md:p-2 rounded-full bg-white/10 hover:bg-white/20 text-white transition-all"
                        >
